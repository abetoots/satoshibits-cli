...
program
  ...
  .option("--lens <lens>", "Evidence lens: docs|code|reconcile (default: docs)")
  .option("--mode <mode>", "Override mode (doc-first|reconcile)")
  .option("--code <paths>", "Source roots for code scanning / agent reading (comma-separated)")
...
program
  ...
  .requiredOption("--tier <level>", "Tier: 1 (foundational), 2 (behavioral), 3 (structural), all (everything)")
  .option("--tier-cumulative", "Include all tiers up to the specified tier")
  .option("--engine <engine>", "Evaluation engine: sdk (toolless) or agent (reads real source)", "sdk")
  .option("--lens <lens>", "Evidence lens: docs|code|reconcile (default: docs)")
  .option("-c, --config <file>", "Path to doc-lint.yaml")
