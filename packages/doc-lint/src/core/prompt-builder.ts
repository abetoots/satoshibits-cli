...
const LENS_FRAMING: Record<Exclude<Lens, "docs">, string> = {
  code:
    "EVALUATION LENS — CODE AUDIT: Read the concern's question as a question about the SYSTEM AS IMPLEMENTED, not about the documentation. Wherever it asks whether something is *documented*, instead determine whether the system *satisfies* that invariant according to the actual source code. Report violations and under-implementations as gaps, each citing real file:line evidence. Do NOT report 'undocumented' as a finding in this lens — absence of docs is out of scope here. Example: for a concern 'all write endpoints must enforce idempotency', if you find `POST /charge` in the code with no idempotency key handling, report a gap against the IMPLEMENTATION (the missing key check at that file:line), NOT against any missing documentation of it.",
  reconcile:
