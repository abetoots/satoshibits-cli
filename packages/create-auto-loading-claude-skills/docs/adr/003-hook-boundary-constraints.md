...
During code review, we discovered that several features in `@satoshibitis/claude-skill-runtime` assumed capabilities that Claude Code hooks don't have. Specifically, the dismissal tracking system (`trackDismissal`, `isSkillSuppressed`, `getDismissalStats`, `ShadowTriggerState`) assumed hooks could receive user feedback.
...
The following boundary-violating code was removed from `@satoshibits/claude-skill-runtime`:
